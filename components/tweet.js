...
export default function Tweet ({ session, currentTweet, sendToParent }) {
  const router = useRouter();
  dayjs.extend(relativeTime);

  const [tweet, setTweet] = useState(currentTweet);
  ...
  async function dislike () {
    ...
    if (res.status === 204) {
      setTweet({ ...tweet, likedBy: tweet.likedBy.filter(item => item !== session.user.id) });
    }
  }
      ...
      <div className="flex flex-auto">
        ...
        <div className="flex flex-col flex-grow ml-4">
          <div className="flex items-center">
            <button onClick={() => router.push(`/${session.user.name}`)} className="font-bold">{session.user.name}</button>
          ...
          <div className="flex items-center space-x-4">
            ...
            <button className="focus:outline-none text-xs text-gray-300 hover:text-black" onClick={() => edit()}>Edit</button>
